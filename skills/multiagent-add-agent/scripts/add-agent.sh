...
# Script lives at: <workspace>/kit/skills/multiagent-add-agent/scripts/add-agent.sh
...
setup_telegram() {
    ...
    if ! confirm "Set up a Telegram bot for $AGENT_NAME?" "n"; then
        log_info "Skipping. Run later with the multiagent-telegram-setup skill."
        return 0
    fi

    local telegram_script="$KIT_DIR/skills/multiagent-telegram-setup/scripts/setup-telegram-agent.py"
